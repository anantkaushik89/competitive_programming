...
void init() {
    ...
    while (!q.empty()) {
        ...
        if (n4 < maxn && !b[n4]) {
            b[n4] = 1, q.push(n4);
            pw[n1][0] = p1, pw[n1][1] = p2, pw[n1][2] = p3, pw[n3][3] = p4 + 1;
        }
    }
...
int get_max_lis(int k) {
    ...
    int len = (p1 ? p1 : 1) + (p2 ? p2 : 1) + (p3 ? p3 : 1) + (p4 ? p4 : 1);
    if (k > near_k) {
...
void solve_parts(int k, int last, int lis) {
    int near_k = get_lim(k, 0, nrem - 1);
    printf("args : k=%d last=%d lis=%d near_k=%d\n", k, last, lis, near_k);
    int p1 = pw[near_k][0], p2 = pw[near_k][1], p3 = pw[near_k][2], p4 = pw[near_k][3];
    int lis_got = (p1 ? p1 : 1) + (p2 ? p2 : 1) + (p3 ? p3 : 1) + (p4 ? p4 : 1);
    int pp1 = p1 ? (p1 * 2) : 1;
    int pp2 = p2 ? (p2 * 3) : 1;
    int pp3 = p3 ? (p3 * 5) : 1;
    int pp4 = p4 ? (p4 * 7) : 1;
